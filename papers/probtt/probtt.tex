...
\textbf{Bayes' rule}: Follows from symmetric disintegration.
...
\begin{theorem}[Bayesian Inversion]
In a Markov category admitting disintegration, Bayes' rule holds:
...
\begin{remark}
This requires the extended weight monoid with division (or just the existence of the alternative disintegration). The key insight: \textbf{Bayes' rule is a structural property of disintegration}, not a numerical formula.
\end{remark}

\subsection{Conditioning in ProbTT}

We add a conditioning primitive:

\begin{gather}
\infer[\textsc{Cond}]
  {\Gamma \vdash a \mid b : A \at{w / v}}
  {\Gamma \vdash (a, b) : A \times B \at{w} \quad \Gamma \vdash b : B \at{v} \quad v \neq \zero}
\end{gather}

Read: ``the weight of $a$ given $b$'' is $w/v$ where $w$ is the joint weight and $v$ is the marginal weight of $b$.

This requires extending $\W$ with division (or working in $\W = (0, 1]$ where division is total).
...
\begin{enumerate}
    ...
    \item \textbf{Bayes' rule}: Follows from symmetric disintegration
    \item \textbf{Independence}: $P(A, B) = P(A) \cdot P(B)$ when disintegration is trivial
\end{enumerate}

All without assuming real numbers---just the categorical structure.
...
\begin{enumerate}
    ...
    \item \textbf{Division}: When $\W$ lacks division, how to handle conditioning?

    \item \textbf{Dependent weights}: What if $w$ depends on $x$ in $(x : A) \to B \at{w(x)}$?

    \item \textbf{Measure zero}: How to handle $P(B) = 0$ in conditioning?

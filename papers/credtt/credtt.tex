...
$c$ is a \emph{post-fixed point} under step $s$ (written $c \leq c \cdot s$) iff applying the proof step does not reduce credence. This is the fundamental validity condition for iterative proofs.
\end{definition}
...
This is what makes CredTT richer than probability theory on $[0,1]$. The Archimedean case (standard real interval) is a \emph{special case} where iteration always degenerates for $s < \one$.
...
\hline
\textbf{\#} & \textbf{Technique} & \textbf{Operator} & \textbf{Stability} & \textbf{Credence} \\
\hline
1 & Direct proof & $T_s \circ T_t = T_{s \cdot t}$ & Compose & $c$ accumulates \\
2 & Cases & Parallel $T_s$ & Both post-fixed & Branches stable \\
3 & Contraposition & $\negc$ antitone & Order reverses & $\negc c_2 \leq \negc c_1$ \\
4 & Reductio & Instability & Unstable$(\negc)$ & $\Rightarrow$ Stable \\
5 & Contrapositive & $\negc \circ T_s$ & Reverses post-fixed & Bounds flip \\
...
11 & $\forall$-intro & $\inf_x T_{s(x)}$ & All post-fixed & $\inf_x c(x)$ \\
12 & $\exists$-intro & $T_{s_a \cdot s_b}$ & Both post-fixed & $c_a \cdot c_b$ \\
13 & Induction & $T_s^n$ & Post-fixed & Degrade/stable \\
14 & Strong ind. & $T_s^k$ & $k$-fold & $c \cdot s^k$ \\
15 & Exhaustion & $\bigwedge T_{s_i}$ & All post-fixed & Min stable \\
...
\begin{enumerate}
    \item \textbf{Fixed points}: The equation $c = c \cdot c$ has solutions $c = \zero$ (since $\zero \cdot \zero = \zero$) and $c = \one$ (since $\one \cdot \one = \one$). These are the only elements, so these are the only fixed points.

    \item \textbf{Operator dichotomy}: For step $s$:
    \begin{itemize}
    ...
    \item \textbf{Classification}: By (3), every credence is either $\zero$ or $\one$. By (1), $\one$ is a robust fixed point and $\zero$ is a vanishing fixed point. This classification is exhaustive.
...
\hline
21 & Stability proofs & Classify $c$ & Post-fixed + positive \\
22 & Credence bounds & Invariant maintenance & $c \geq l$ preserved by $T_s$ \\
23 & Robustness & Lipschitz on $T_s$ & $|T_s(c) - T_s(c')| \leq L|c - c'|$ \\
...
\begin{itemize}
    \item \textbf{Tarski's Fixed Point Theorem} \citep{tarski1955lattice}: Monotone functions on complete lattices have fixed points. CredTT's proof operators $T_s$ are monotone, and the credence algebra (when complete) admits fixed points by Tarski's theorem.
...
\begin{itemize}
    ...
    \item \textbf{Modal $\mu$-Calculus}: Least and greatest fixed points characterize temporal properties. CredTT's post-fixed points ($c \leq c \cdot s$) are analogous to pre-fixed points in modal logic.

    \item \textbf{Inductive Definitions} \citep{aczel1977inductive}: Aczel's theory of inductive definitions uses fixed points on set-theoretic structures. CredTT's induction principle requires the credence to be a post-fixed point.
\end{itemize}

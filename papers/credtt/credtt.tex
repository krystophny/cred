...
We replace the constant truth values $\top$ and $\bot$ with \emph{stability regions} near credence extrema. This is not metaphorical: stability is a meta-theoretic property expressing persistence of inhabitation under small degradations.

\subsection{From Points to Regions}

Classical logic treats truth values as points: $\top$ or $\bot$, no intermediate values. CredTT replaces this with a continuous spectrum $[0,1]$. The key insight is that classical proof techniques don't disappear---they become \emph{stability theorems} about regions near the extrema.

\begin{definition}[Stable Inhabitation]
$\Gamma \vdash A$ is \emph{stable} (written $\mathrm{Stable}_\one$) iff there exists $c_0 < \one$ such that for all $c \geq c_0$, there exists a derivation $\Gamma \vdash a : A \at{c}$.
\end{definition}

\begin{definition}[Unstable Inhabitation]
$\Gamma \vdash A$ is \emph{unstable} (written $\mathrm{Unstable}_\zero$) iff for all $c > \zero$, there exists $c' < c$ such that no derivation $\Gamma \vdash a : A \at{c'}$ exists.
\end{definition}

These are \emph{meta-properties}, like termination, not judgments inside CredTT. They capture whether inhabitation persists under degradation (stable) or collapses under strengthening (unstable).
...
For mathematical induction with base case @ $c_b$ and step @ $c_s$:
\begin{itemize}
    \item $P(0) \at{c_b}$
    \item $P(n) \Rightarrow P(n+1) \at{c_s}$
    \item $P(n) \at{c_b \cdot c_s^n}$
\end{itemize}
If $c_s = 0.99$, then $P(100) \at{c_b \cdot 0.99^{100}} = c_b \cdot 0.366$. Even ``almost certain'' steps degrade over iterations! Only $c_s = \one$ (contractive step) preserves credence.

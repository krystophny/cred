...
\begin{abstract}
We define a type theory where judgments carry \emph{credences} from a De Morgan algebra. The algebra has multiplication, complement, zero, one, and order---with disjunction derived via De Morgan duality. When $\C = \{\zero, \one\}$, we recover Martin-L\"of Type Theory. When $\C = {[0,1]}$, we get probabilistic typing where \emph{credence multiplication} captures conditioning: the elimination rules multiply credences, implementing the chain rule $P(A,B) = P(A) \cdot P(B|A)$ without requiring explicit division. The key insight: types and logic \emph{emerge} from credence structure. Note: This paper presents the \emph{design} of CredTT; a full formal verification remains future work (see Section~\ref{sec:status}).
\end{abstract}
...
\begin{theorem}[De Morgan Laws]\label{thm:demorgan}
\begin{align}
\negc(c \cdot v) &= \negc c \vee \negc v \\
\negc(c \vee v) &= \negc c \cdot \negc v
\end{align}
...
\begin{enumerate}
    \item $\negc(c \cdot v) = \negc c \vee \negc v$: By definition, $\negc c \vee \negc v = \negc(\negc\negc c \cdot \negc\negc v)$. Using involution $\negc\negc x = x$, this equals $\negc(c \cdot v)$.
    \item $\negc(c \vee v) = \negc c \cdot \negc v$: By definition, $c \vee v = \negc(\negc c \cdot \negc v)$. Taking complement: $\negc(c \vee v) = \negc\negc(\negc c \cdot \negc v) = \negc c \cdot \negc v$ by involution.
\end{enumerate}
Formally verified in Agda: \texttt{agda/CredTT/Credence.agda}, module \texttt{DeMorganLaws}, theorems \texttt{demorgan-\textperiodcentered} and \texttt{demorgan-\(\vee\)}.
\end{proof}
...
\textbf{Formalization status}: The conditional judgment $b \mid a : B \at{v}$ is a \emph{meta-level} notion used to express the chain rule. The Agda formalization does not include a term-level conditional operator $(\_ \mid \_)$; instead, the chain rule is captured by the Sigma-type introduction rule which computes joint credence as the product of component credences. Full term-level conditioning would require extending the syntax with a conditional term former and appropriate computation rules---this remains future work.
...
\emph{Implementation note: The conditioning relation $b \mid a$ is captured implicitly through credence multiplication in Pi-Elim and Sigma-Intro rules, rather than as a distinct syntactic form. See check.ml for the credence propagation implementation.}

%=============================================================================
\section{MLTT as the $\{\zero, \one\}$ Case}
%=============================================================================

\begin{theorem}[MLTT Embedding]\label{thm:mltt-embedding}
...
\begin{proof}
We construct a bidirectional correspondence between MLTT and CredTT judgments:

\textbf{Forward direction} (\texttt{embed}): Every MLTT judgment $\Gamma \vdash_\textsf{MLTT} t : A$ yields a CredTT judgment $\Gamma \vdash t : A \at{\one}$. The proof is by induction on the MLTT derivation:
\begin{itemize}
    \item Variable: $\Gamma \vdash_\textsf{MLTT} x_i : A_i$ embeds to $\Gamma \vdash x_i : A_i \at{\one}$ by \textsc{t-var}.
    \item Lambda: If $\Gamma, A \vdash_\textsf{MLTT} b : B$ embeds to $\Gamma, A \vdash b : B \at{\one}$, then $\Gamma \vdash_\textsf{MLTT} \lambda b : A \to B$ embeds to $\Gamma \vdash \lambda b : A \to B \at{\one}$ by \textsc{t-lam}.
    \item Application: Given embeddings of $f : A \to B \at{\one}$ and $a : A \at{\one}$, the application has credence $\one \cdot \one = \one$ by \textsc{t-app}.
    \item Other type formers (pairs, injections, case, refl, J) follow the same pattern.
\end{itemize}

\textbf{Backward direction} (\texttt{collapse}): Every CredTT judgment $\Gamma \vdash t : A \at{\one}$ yields an MLTT judgment $\Gamma \vdash_\textsf{MLTT} t : A$. Since $\one \cdot \one = \one$, sub-judgments at credence $\one$ remain at credence $\one$, and the derivation translates directly.

\textbf{Round-trip}: The composition $\texttt{collapse} \circ \texttt{embed}$ is the identity on MLTT derivations.

Formally verified in Agda: \texttt{agda/CredTT/MLTT.agda}, functions \texttt{embed}, \texttt{collapse}, and \texttt{embed-collapse}.
\end{proof}
...
\emph{Status: Definitional sketch. These definitions describe the intended categorical structure; coherence conditions and functoriality have not been formally verified. Full verification requires checking associativity/identity laws and naturality of structural morphisms.}
...
The key insight: proofs are not static objects but \emph{dynamical systems} on the credence algebra. This perspective connects CredTT to the theory of monotone operators on ordered algebraic structures \citep{tarski1955lattice, cousot1979constructive}.

\begin{definition}[Proof Operator]
Each proof step at credence $s$ induces the operator $T_s : \C \to \C$ where $T_s(c) = c \cdot s$.
\end{definition}

Since multiplication preserves order in each argument, $T_s$ is a monotone operator on the ordered credence algebra. Stability is characterized by fixed-point properties:

\begin{definition}[Pre-Fixed Point]
$c$ is a \emph{pre-fixed point} under step $s$ (written $c \leq c \cdot s$) iff applying the proof step does not reduce credence. In standard lattice theory, $x \leq f(x)$ defines pre-fixed points. This is the fundamental validity condition for iterative proofs.
\end{definition}

\begin{definition}[Invariant]
$c$ is \emph{invariant} under step $s$ (written $c = c \cdot s$) iff it is an exact fixed point. Invariant credences are \emph{stable} under the dynamics.
\end{definition}

\begin{definition}[Degenerating]
$c$ \emph{degenerates} under step $s$ iff $\inf_n (c \cdot s^n) = \zero$. Degenerating credences eventually collapse to impossibility.
\end{definition}
...
\textbf{Critical}: We do NOT assume Archimedeanicity. In non-Archimedean algebras:
\begin{itemize}
    \item $s < \one$ does NOT imply $s^n \to \zero$
    \item Interior idempotents can exist (e.g., $e$ with $\zero < e < \one$ and $e \cdot e = e$)
    \item Multiple fixed points are possible
    \item Credence sequences may plateau at interior values
\end{itemize}

\begin{example}[Credence algebra with interior idempotent]
Consider the three-element algebra $\C = \{\zero, e, \one\}$ with order $\zero < e < \one$, multiplication $e \cdot e = e$, $e \cdot \one = e$, $\zero \cdot x = \zero$, $\one \cdot x = x$, and complement $\negc e = e$ (the unique fixed point of negation). This is a valid credence algebra where $e$ is an interior idempotent: $\zero < e < \one$ and $e \cdot e = e$.
\end{example}

This is what makes CredTT richer than probability theory on $[0,1]$. The Archimedean case (standard real interval) is a \emph{special case} where iteration always degenerates for $s < \one$, since the only idempotents are $0$ and $1$.

\subsection{Interior Credences as First-Class Citizens}

In classical logic, only $\zero$ and $\one$ exist. In probability theory on $[0,1]$, interior values are ``approximations'' that converge to extremes. In CredTT:

\begin{center}
\begin{tabular}{l|l}
\textbf{Credence Type} & \textbf{Status} \\
\hline
$c = \one$ (certainty) & Maximal fixed point, robust \\
$c = \zero$ (impossibility) & Minimal fixed point, vanishing \\
$\zero < c < \one$ (interior) & \textbf{First-class}, may be stable if idempotent \\
\end{tabular}
\end{center}

Interior credences are not ``partial truth'' or ``approximations''---they are legitimate epistemic states. The G\"odel sentence at credence $\tfrac{1}{2}$ is not ``partially true''; it has a determinate credence that is neither $\zero$ nor $\one$.
...
Every classical proof technique has a CredTT analog expressed in terms of stability. The table below shows all 20 classical techniques with their operator dynamics, stability conditions, and credence behavior:

\begin{center}
\small
\begin{tabular}{|c|l|l|l|l|}
\hline
\textbf{\#} & \textbf{Technique} & \textbf{Operator $T_s$} & \textbf{Validity Condition} & \textbf{Result Credence} \\
\hline
1 & Direct proof & $T_s \circ T_t = T_{s \cdot t}$ & Compose & $c$ accumulates \\
2 & Cases & Parallel $T_s$ & Both pre-fixed & Branches stable \\
3 & Contraposition & $\negc$ antitone & Order reverses & $\negc c_2 \leq \negc c_1$ \\
4 & Reductio & Instability & Unstable$(\negc)$ & $\Rightarrow$ Stable \\
5 & Contrapositive & $\negc \circ T_s$ & Reverses pre-fixed & Bounds flip \\
6 & Ex falso & $T_s(\zero) = \zero$ & $\zero$ absorbs & Unconstrained \\
7 & Vacuous & $\zero \cdot s = \zero$ & Degenerate & Vacuous \\
8 & Deduction & $\lambda$ preserves & Same dynamics & $c$ preserved \\
9 & Modus ponens & $T_{s_1 \cdot s_2}$ & Multiply & $c_1 \cdot c_2$ \\
10 & Syllogism & $T_{s_1} \circ T_{s_2}$ & Compose & $c_1 \cdot c_2$ \\
11 & $\forall$-intro & $\inf_x T_{s(x)}$ & All pre-fixed & $\inf_x c(x)$ \\
12 & $\exists$-intro & $T_{s_a \cdot s_b}$ & Both pre-fixed & $c_a \cdot c_b$ \\
13 & Induction & $T_s^n$ & Pre-fixed & Degrade/stable \\
14 & Strong ind. & $T_s^k$ & $k$-fold & $c \cdot s^k$ \\
15 & Exhaustion & $\bigwedge T_{s_i}$ & All pre-fixed & Min stable \\
16 & Construction & $T_{s_w \cdot s_p}$ & Witness+prop & $c_w \cdot c_p$ \\
17 & Refutation & Iterate to $\zero$ & Degenerates & $\inf_n = \zero$ \\
18 & Rewriting & $T_{\mathsf{Id}}$ & Id stable & Preserved \\
19 & Analogy & $T_s$, $s < \one$ & Explicit weak & Visible \\
20 & Structural & Conditional & See below & Costs explicit \\
\hline
\end{tabular}
\end{center}
...
Induction is valid iff $c$ is a \emph{pre-fixed point} of the step operator:
\begin{itemize}
...
When credences are restricted to the Boolean algebra $\{\zero,\one\}$, the dynamics trivialize completely. This is the \emph{degenerate case} where classical logic emerges.

\begin{theorem}[Collapse]
When credences are restricted to $\{\zero,\one\}$:
\begin{enumerate}
    \item \textbf{Only two fixed points}: $\zero$ (vanishing) and $\one$ (robust)
    \item \textbf{Dynamics trivialize}: $T_\one = \mathrm{id}$, $T_\zero = \mathrm{const}_\zero$
    \item \textbf{No interior}: Every credence is extremal
    \item \textbf{Classification exhaustive}: Robust$(\one)$ or Vanishing$(\zero)$
    \item \textbf{MLTT recovery}: CredTT judgments correspond exactly to MLTT judgments
\end{enumerate}
\end{theorem}

\begin{proof}[Proof sketch (order-theoretic; see Collapse.agda)]
\textbf{Formalization status}: The Agda file \texttt{Collapse.agda} proves claims (1)--(4) mechanically for the Boolean De Morgan algebra. The MLTT correspondence (5) is demonstrated structurally via judgment records but full integration with the term language requires additional work (tracked in GitHub issue \#57).

In the Boolean algebra $\{\zero,\one\}$:
\begin{enumerate}
    \item \textbf{Fixed points of $T_s$}: For any operator $T_s(c) = c \cdot s$:
    \begin{itemize}
        \item $T_s(\zero) = \zero \cdot s = \zero$, so $\zero$ is always a fixed point
        \item $T_s(\one) = \one \cdot s = s$, so $\one$ is a fixed point iff $s = \one$
    \end{itemize}
    Since $\{\zero,\one\}$ has only two elements: $\zero$ is a fixed point of every $T_s$, and $\one$ is a fixed point of $T_\one$. These are the only candidates and hence the only fixed points.

    \item \textbf{Operator dichotomy}: For step $s \in \{\zero, \one\}$:
    \begin{itemize}
        \item If $s = \one$: $T_\one(c) = c \cdot \one = c$ (identity)
        \item If $s = \zero$: $T_\zero(c) = c \cdot \zero = \zero$ (annihilation)
    \end{itemize}
    No other behavior is possible.

    \item \textbf{No interior}: Suppose $\zero < c < \one$. But in $\{\zero,\one\}$, there are no such elements. The order $\zero \leq \one$ has no intermediate points.

    \item \textbf{Classification}: By (3), every credence is either $\zero$ or $\one$. By (1), $\one$ is a robust fixed point (under $T_\one$) and $\zero$ is a vanishing fixed point (under all $T_s$). This classification is exhaustive.

    \item \textbf{MLTT}: Judgments $\Gamma \vdash a : A \at{\one}$ correspond to MLTT judgments $\Gamma \vdash a : A$. Judgments at credence $\zero$ are vacuously satisfied (no computational content).
\end{enumerate}
\end{proof}

This explains why classical logic appears to have exact truth values: it is the degenerate limit where:
\begin{itemize}
    \item Neighbourhoods collapse to singletons
    \item The G\"odel fixed point $c = \negc c$ has no solution (undecidability)
    \item Proof dynamics become trivial (identity or annihilation)
\end{itemize}

CredTT reveals the underlying continuous structure that classical logic hides.
...
CredTT enables 8 proof techniques with \emph{no classical analog}. These exploit the dynamics framework:

\begin{center}
\small
\begin{tabular}{|c|l|l|l|}
\hline
\textbf{\#} & \textbf{Technique} & \textbf{Operator/Dynamics} & \textbf{Stability Condition} \\
\hline
21 & Stability proofs & Classify $c$ & Pre-fixed + positive \\
22 & Credence bounds & Invariant maintenance & $c \geq l$ preserved by $T_s$ \\
23 & Robustness & Order-preserving $T_s$ & $c \leq c' \Rightarrow T_s(c) \leq T_s(c')$ \\
24 & Degeneracy & Iteration analysis & Rate of $c \cdot s^n \to \zero$ \\
25 & Contractivity & $T_s$ non-expanding & $T_s(c) \leq c$ \\
26 & Factoring & Decompose $c = h \cdot l$ & High-cred core isolated \\
27 & Dual proofs & Upper + lower bounds & Squeeze to exact value \\
28 & Limit theorems & $\lim_{n \to \infty} c \cdot s^n$ & Convergence rate \\
\hline
\end{tabular}
\end{center}
...
\subsection{Monotone Dynamical Systems}

CredTT's proof operators form a \emph{monotone dynamical system} on the credence algebra. The relevant literature includes:

\begin{itemize}
    \item \textbf{Tarski's Fixed Point Theorem} \citep{tarski1955lattice}: Monotone functions on complete lattices have fixed points. When the credence algebra forms a complete lattice (e.g., $[0,1]$ with standard order), CredTT's proof operators $T_s$ are monotone and admit fixed points by Tarski's theorem. Note: the general credence algebra axioms do not assume completeness; this application requires the additional assumption.

    \item \textbf{Abstract Interpretation} \citep{cousot1979constructive}: Cousot and Cousot's framework uses Galois connections and fixed-point iteration on abstract domains. CredTT's stability analysis is analogous: iteration of $T_s$ converges to fixed points characterizing proof validity.

    \item \textbf{Iterative Methods}: The iteration $c, c \cdot s, c \cdot s^2, \ldots$ is a discrete dynamical system. Convergence analysis (when it degenerates to $\zero$ vs.\ stabilizes at an interior fixed point) connects to classical dynamical systems theory.
\end{itemize}

\subsection{Fixed Point Theory}

The fixed-point perspective on logic has deep roots:

\begin{itemize}
    \item \textbf{Kripke Fixed Points} \citep{kripke1975outline}: Kripke's theory of truth uses fixed points to handle self-reference. CredTT's G\"odel sentence at $c = \tfrac{1}{2}$ is a fixed point of the negation operator.

    \item \textbf{Modal $\mu$-Calculus}: Least and greatest fixed points characterize temporal properties. CredTT's pre-fixed points ($c \leq c \cdot s$) are analogous to pre-fixed points in modal logic.

    \item \textbf{Inductive Definitions} \citep{aczel1977inductive}: Aczel's theory of inductive definitions uses fixed points on set-theoretic structures. CredTT's induction principle requires the credence to be a pre-fixed point.
\end{itemize}

\subsection{Non-Archimedean Structures}

CredTT deliberately avoids the Archimedean assumption:

\begin{itemize}
    \item \textbf{Non-standard Analysis} \citep{robinson1966nonstandard}: Robinson's hyperreals contain infinitesimals and infinite numbers. Non-Archimedean credence algebras can have similar structure.

    \item \textbf{Valued Fields}: $p$-adic numbers and other non-Archimedean fields have discrete valuations. CredTT's credence algebras may similarly have ``gaps'' allowing interior fixed points.

    \item \textbf{Ultrametric Spaces}: In ultrametric spaces, $d(x,z) \leq \max(d(x,y), d(y,z))$. Non-Archimedean credence algebras may satisfy analogous ``strong'' properties.
\end{itemize}

The key insight: in non-Archimedean settings, $s < \one$ does NOT imply $s^n \to \zero$. This allows interior idempotents ($e = e \cdot e$ with $\zero < e < \one$), enabling genuinely stable interior credences.
...
\subsection{Positioning vs.\ QTT and Granule}

Quantitative Type Theory (QTT) \citep{atkey2018syntax} and Granule \citep{orchard2019quantitative} are the closest related work in graded type theory:

\begin{center}
\begin{tabular}{l|l|l|l}
\textbf{Aspect} & \textbf{QTT/Granule} & \textbf{CredTT} & \textbf{Key Difference} \\
\hline
Algebra & Semiring $(0,1,+,\times)$ & De Morgan $(\zero,\one,\cdot,\negc)$ & No primitive $+$ \\
Interpretation & Resource usage & Credence/certainty & Epistemic vs.\ operational \\
Dynamics & Not emphasized & Central (operators) & Fixed points matter \\
MLTT relation & Parameterized & Derived as limit & Direction of generalization \\
Implication & Standard & Conditioning & No material implication \\
\end{tabular}
\end{center}

Both QTT/Granule and CredTT treat grades as primitive. The philosophical difference is interpretive: QTT asks ``how much resource?'' while CredTT asks ``how certain?'' The technical difference is algebraic: CredTT's De Morgan structure lacks addition but gains negation.

\subsection{What CredTT Unifies}

CredTT sits at the intersection of:
\begin{itemize}
    \item Monotone dynamical systems (operators, fixed points)
...
But differs by: (i) dynamics-first semantics, (ii) non-Archimedean generality, (iii) conditioning replaces implication, (iv) MLTT as the $\{\zero,\one\}$ collapse.
...
\begin{theorem}[Graded Incompleteness]\label{thm:graded-incompleteness}
...
The Agda formalization (\texttt{agda/CredTT/Reflection.agda}) \textbf{postulates} rather than proves the key components of this theorem:
\begin{itemize}
    \item \textbf{Postulated}: The G\"odel sentence $G$ and its self-reference property $\mathsf{Prov}(G) = \negc(\mathsf{Prov}(G))$ (requires full arithmetic encoding)
    \item \textbf{Postulated}: Existence and uniqueness of the negation fixpoint $c = \negc c$ (holds in $[0,1]$; may fail in non-Archimedean algebras)
    \item \textbf{Derived}: Given the above postulates, $G$-credence = $\frac{1}{2}$ follows by fixpoint uniqueness
\end{itemize}
A complete formalization would require: (1) G\"odel encoding of CredTT syntax in Agda, (2) diagonal lemma construction, and (3) proof that $[0,1]$ with $\negc c = 1-c$ admits a unique negation fixpoint. This is standard metamathematics but substantial work. See the module header in \texttt{Reflection.agda} for detailed status of each postulate.
\end{remark}
...
\begin{theorem}[Graded Second Incompleteness]\label{thm:graded-second-incompleteness}
CredTT can prove $\mathsf{Con}_c(\text{CredTT})$ for some $c < \one$, but not for $c = \one$.
\end{theorem}

\begin{remark}[Formalization Status]
This theorem is a \textbf{conjecture} based on the structure of G\"odel's Second Incompleteness Theorem adapted to graded settings. The Agda formalization (\texttt{agda/CredTT/Reflection.agda}) \textbf{postulates} rather than proves the key components:
\begin{itemize}
    \item The G\"odel sentence existence (requires full arithmetic encoding)
    \item The negation fixpoint $c = \negc c$ existence (proven for $[0,1]$, fails for Bool)
    \item The self-referential credence assignment
\end{itemize}
A full proof would require formalizing G\"odel encoding of CredTT syntax and proving the derivability conditions. The proof sketch below outlines the argument structure.
\end{remark}
...
\textbf{Axiom Schema} (Choice Credence Scaling):
\begin{enumerate}
...
\begin{remark}[Formalization Status]
These are \textbf{axiom schemas}, not theorems. The Agda formalization (\texttt{agda/CredTT/GradedChoice.agda}) explicitly marks these as \texttt{postulate}s:
\begin{itemize}
    \item \texttt{ExistsAt} is a primitive notion bridging credences to existence
    \item \texttt{finite-choice} is postulated as constructively valid
    \item \texttt{countable-choice-credence} and \texttt{countable-choice-limit} are postulated
    \item The limit credence depends on algebra-specific convergence properties
\end{itemize}
The intuitions below explain \emph{why} these axioms are reasonable, not proofs.
\end{remark}
...
\textbf{Conjecture} (Graded Banach-Tarski):
The Banach-Tarski decomposition exists at some credence $c_{BT} < \one$.

\begin{remark}[Formalization Status]
This is a \textbf{conjecture}, not a theorem. The Agda formalization (\texttt{agda/CredTT/GradedChoice.agda}) explicitly marks the components as \texttt{postulate}s:
\begin{itemize}
    \item \texttt{banach-tarski-credence : C} is postulated
    \item \texttt{BanachTarskiDecomp : Set} is an abstract type
    \item \texttt{bt-decomposition-exists} asserts existence at the postulated credence
    \item \texttt{bt-not-realizable} asserts the credence is strictly less than $\one$
\end{itemize}
A proof would require formalizing Banach-Tarski in CredTT (measure theory), showing the construction uses uncountable choice, and computing the resulting credence.
\end{remark}
...
\begin{enumerate}
    ...
    \end{itemize}
    \item \textbf{Agda formalization} (\texttt{agda/CredTT/}): Syntax, typing rules with credence multiplication, MLTT embedding theorem. Note: (a) Agda has credence-annotated \emph{typing rules} (in \texttt{Judgment.agda}) but not a \emph{type-checking algorithm}---the OCaml implementation provides the algorithmic type checker while Agda provides the specification; (b) The MLTT embedding (\texttt{MLTT.agda}) proves one direction of the round-trip (\texttt{embed-collapse}); the other direction (\texttt{collapse-embed}) is not yet proven---see Issue \#76
\end{enumerate}
...
\begin{itemize}
    \item Variables are always assigned credence $\one$ (see \texttt{check.ml})
...
\subsection{Formalization Status (Issue \#138)}

The Agda formalization (\texttt{agda/CredTT/}) provides \textbf{structure}, not complete proofs:

\begin{itemize}
    \item \textbf{50+ postulates}: Core metatheorems (substitution, soundness, completeness) are postulated, not proven
    \item \textbf{Axiom fields}: Key properties like $\negc$-antitone are axioms that each algebra instance must satisfy
    \item \textbf{Concrete models}: BoolDM is fully proven; IntervalDM relies on arithmetic postulates
\end{itemize}

\noindent\textbf{What IS proven:}
\begin{itemize}
    \item De Morgan algebra structure and derived operations
    \item Dynamics-based stability classification
    \item MLTT embedding in one direction (\texttt{embed-collapse})
    \item Basic type formation rules with credence tracking
\end{itemize}

\noindent\textbf{What is CONJECTURED (postulated):}
\begin{itemize}
    \item Substitution lemma (Issue \#115)
    \item Subject reduction/progress (Issues \#186, \#187)
    \item Pavelka completeness (Issue \#99)
    \item G\"odel sentence existence at credence $\frac{1}{2}$ (Issue \#124)
\end{itemize}

This paper should be read as a \textbf{design proposal} for CredTT, with the Agda files serving as executable specifications. Full verification is future work requiring 4000+ lines of additional proof.
...
\begin{enumerate}
    \item \textbf{Soundness} (critical): We have not proven the standard metatheoretic properties:
    \begin{itemize}
        \item \emph{Substitution lemma}: If $\Gamma \vdash a : A \at{c}$ and $\Gamma, x : A \vdash t : B \at{d}$, then $\Gamma \vdash t[a/x] : B[a/x] \at{?}$. The credence interaction under substitution requires careful analysis---does $d$ depend on $c$?
        \item \emph{Subject reduction} (type preservation): If $\Gamma \vdash t : A \at{c}$ and $t \to t'$, then $\Gamma \vdash t' : A \at{c'}$ for some $c' \geq c$. We conjecture credence is preserved or improved by reduction, but this is not proven.
        \item \emph{Progress}: A well-typed term at credence $c > \zero$ is either a value or can reduce. Vacuous terms (credence $\zero$) need not make progress.
    \end{itemize}
    The Agda formalization (\texttt{agda/CredTT/Substitution.agda}) includes substitution operations but postulates identity substitution lemmas. Full soundness remains open---see GitHub issues \#185 (substitution lemma), \#186 (subject reduction), and \#187 (progress).

(* Unit tests for weight inference *)
...
let () =
  ...
  test "mul with Infer simplifies correctly" (
    let w = simplify (mul (Infer 0) one) in
    match w with
    | Infer 0 -> true
    | _ -> false
  );
  ...
  test "unify Infer with Var" (
    match unify [] (Infer 2) (Var "w") with
    | Unified s ->
  ...
  test "apply_subst through Mul" (
    let s = [(0, One)] in
    let w = apply_subst s (Mul (Infer 0, Var "w")) in
    equal w (Var "w")
  );

  test "apply_subst through Neg" (
    let s = [(0, Zero)] in
    let w = apply_subst s (Neg (Infer 0)) in
    equal w One
  );
  ...
  add_constraint ctx2 (Infer 0) (Var "w");
  ...
  test "solve_inference with multiple constraints" (
    ...
    | Ok s ->
        ...
        (match v0, v1 with
         | Some (Var "w"), Some One -> true
         | _ -> false)
    | Error _ -> false
  );

  (* Test finalize_weight *)
  reset_infer ();
  let ctx3 = create_infer_ctx () in
  add_constraint ctx3 (Infer 0) Zero;
  (match solve_inference ctx3 with
   | Ok _ ->
       test "finalize_weight resolves Infer to value" (
         let w = finalize_weight ctx3 (Mul (Infer 0, Var "x")) in
         equal w Zero
       )
   | Error _ -> test "finalize_weight resolves Infer to value" false);
  ...
  (* Test infer_derivation_weight *)
  test "infer_derivation_weight preserves weight for known steps" (
    let w = Var "w" in
    equal (infer_derivation_weight ~from_weight:w ~step:"algebra") w &&
    equal (infer_derivation_weight ~from_weight:w ~step:"substitution") w &&
    equal (infer_derivation_weight ~from_weight:w ~step:"self_reference") w
  );

  Printf.printf "\nAll weight inference tests passed!\n"

...
\subsection{Scope of the Collapse}
\label{sec:scope}

The Kleene algebra is not an arbitrary choice of target: it is the unique
natural three-valued quotient of Cred.
No analogous homomorphism exists for the other standard many-valued algebras.
...
The obstacle is that $\{0, 1\}$ has no fixed point of $c \mapsto 1 - c$, while
$[0,1]$ has one at $\tfrac{1}{2}$.
The Boolean restriction (Section~\ref{sec:connections}) works in the other direction:
$\{0, 1\}$ embeds into $[0,1]$ as a subalgebra, but there is no retraction.
...
Let $c = 1/\!\sqrt{2}$, so $c^2 = \tfrac{1}{2}$.
The conjunction condition gives:
\[
\varphi(\tfrac{1}{2}) = \varphi(c^2) = \max(2\varphi(c) - 1,\; 0) = \tfrac{1}{2}
\]
Hence $2\varphi(c) - 1 = \tfrac{1}{2}$, giving $\varphi(c) = \tfrac{3}{4}
\notin \{0, \tfrac{1}{2}, 1\}$.
\end{proof}

The three impossibility results share a common structure: the negation fixed point
at $\tfrac{1}{2}$ propagates constraints that only the Kleene algebra can absorb.
...
Kleene conjunction $\min(\tfrac{1}{2}, \tfrac{1}{2}) = \tfrac{1}{2}$, and the
mismatch propagates to irrational values.

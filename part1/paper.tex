...
\end{theorem}
\begin{proof}
If $c_1 \conjc \text{evidence} = c_2 \conjc \text{evidence} = \text{joint}$
and evidence $> 0$, then $c_1 = c_2$.
...
\end{theorem}
\begin{proof}
Case analysis on whether each $c_i$ equals $0$, $1$, or lies in $(0,1)$.
The key cases are:
\begin{itemize}
\item If $c_1 = 0$ or $c_2 = 0$, then $c_1 \conjc c_2 = 0$, and $\min(v_1, v_2) = 0$.
\item If $c_1 = 1$, then $c_1 \conjc c_2 = c_2$, and $\min(1, v_2) = v_2$.
\item If $0 < c_1, c_2 < 1$, then $0 < c_1 \conjc c_2 < 1$, so both sides
collapse to $\tfrac{1}{2}$, and $\min(\tfrac{1}{2}, \tfrac{1}{2}) = \tfrac{1}{2}$.
\end{itemize}
Similar analysis for negation and disjunction.
...
\textbf{Conditioning differs from RM3 implication.}
RM3 defines implication as $a \to b = \max(\negc a, b)$, giving $0 \to b = 1$
for all $b$ (ex falso).
Cred conditioning at evidence $= 0$ is unconstrained.

\begin{center}
\begin{tabular}{c|ccc}
\multicolumn{4}{c}{\textbf{RM3 Implication}} \\
& 0 & $\tfrac{1}{2}$ & 1 \\
\hline
0 & 1 & 1 & 1 \\
$\tfrac{1}{2}$ & $\tfrac{1}{2}$ & $\tfrac{1}{2}$ & 1 \\
...
\qquad
\begin{tabular}{c|ccc}
\multicolumn{4}{c}{\textbf{Cred Conditioning}} \\
\textit{evidence} & \multicolumn{3}{c}{\textit{joint}} \\
& 0 & $\tfrac{1}{2}$ & 1 \\
\hline
0 & * & * & * \\
$\tfrac{1}{2}$ & 0 & $\dagger$ & 1 \\
...
$\dagger$ = depends on joint (e.g., $\tfrac{1}{4}/\tfrac{1}{2} = \tfrac{1}{2}$
or $\tfrac{1}{2}/\tfrac{1}{2} = 1$)}
\normalsize
...
Conditioning remains distinct from material implication: material implication
gives $0 \to B = 1$, while conditioning at $0$ is unconstrained.
...
\end{proposition}
\begin{proof}
Negation at $c = \tfrac{1}{2}$ gives $\varphi(\tfrac{1}{2}) = \tfrac{1}{2}$.
Pick any $c \in (0,1)$ with $c^2 = \tfrac{1}{2}$ (e.g.\ $c = 1/\!\sqrt{2}$).
Conjunction preservation gives
$\varphi(\tfrac{1}{2}) = \mathrm{conj}_{\text{\L}}(\varphi(c), \varphi(c))$.
But $\mathrm{conj}_{\text{\L}}(v,v) = \max(2v-1,0)$ takes values $0, 0, 1$
for $v \in \{0, \tfrac{1}{2}, 1\}$ respectively---none equals $\tfrac{1}{2}$.
\end{proof}
...
Product fuzzy logic shares the product and De Morgan dual with Cred but diverges
on implication: residuated implication forces $0 \to y = 1$, while Cred's
chain-rule conditioning is unconstrained at zero.
Neither is a special case of the other.
Cred shares with relevant logic the property that conditioning/implication from
impossibility is not forced to yield $1$; the mechanisms differ (chain rule
constraint vs.\ syntactic variable-sharing).
...
Cred shares this triplet but replaces the residuated implication with chain-rule
conditioning---the point of divergence.
...
The formalization comprises approximately 870 lines covering:
...
The divergence is in conditioning: where product fuzzy logic uses residuated
implication (forcing ex falso), Cred uses the chain rule.
The three-valued collapse establishes a surjective homomorphism to the Kleene
algebra---the shared foundation of K$_3$, LP, and RM3---and no analogous
collapse exists for \L ukasiewicz, G\"odel, or classical targets.
Conditioning remains distinct from the implication used in these three-valued logics.

...
product residuated implication as the unique Bayes-consistent arrow among the
standard logic implications (RM3, G\"odel, product residuated).
...
values---which is what makes the collapse homomorphism to the Kleene algebra
...
$\conjc$ is neither idempotent nor absorptive (see below).
...
Every copula $C$ (a symmetric function $[0,1]^2 \to [0,1]$ satisfying the
boundary conditions $C(a,0) = 0$, $C(a,1) = a$ and the 2-increasing
property~\cite{nelsen2006}) yields a Bayes-consistent arrow via
$I_C(a,b) = C(a,b)/a$.
...
both directions selects the product residuated implication as the unique
Bayes-consistent arrow among RM3, G\"odel, and product residuated
implications, with every copula yielding a Bayes-consistent arrow
(machine-checked on three values; pen-and-paper on $[0,1]$).
...
the standard logic implications (RM3, G\"odel, product residuated):
RM3's arrow fails even on three values, and G\"odel's fails on the
continuum.

...
\item \textbf{Path dependence at zero} (Proposition~\ref{thm:path-dependence}):
  ...
  $\to 0$ depends on the ratio of joint to evidence (loosely analogous
  to the Borel--Kolmogorov paradox, though the mechanisms differ).
  Ex falso requires an added convention.
\item \textbf{Collapse to Kleene} (Theorem~\ref{thm:collapse}):
  The quotient by $\{0\},(0,1),\{1\}$ is the Kleene lattice (routine);
  no analogous collapse exists for G\"odel, \L ukasiewicz, or classical logic
  (the impossibility results are the substantive content).
...
idempotent, but $\conjc$ is not idempotent (Lemma~\ref{thm:idempotence})
and distributivity fails (Lemma~\ref{thm:nondistrib}).
...
Lean: \texttt{min\_copula\_unique}, \texttt{truth\_functional\_forces\_min},
\texttt{min\_nontrivial\_conditioning}.
\end{proof}

Conditions (i)--(v) are the defining properties of a symmetric copula that
is idempotent.
...
That complement on $[0,1]$ fixes $\tfrac{1}{2}$ is standard in many-valued
logic~\cite{hajek1998,gottwald2001}.
We record the facts here because the collapse results in
Section~\ref{sec:collapse} depend on them explicitly, and the Lean
formalization tracks the dependency.

% Lean: liar_fixed_point
\begin{lemma}[Negation Fixed Point]
\label{thm:liar}
...
\begin{lemma}[Uniqueness of Fixed Point]
...
If one imposes $\cred(L) = \negc \cred(L)$ for a self-negating sentence $L$,
the unique solution is $\cred(L) = \tfrac{1}{2}$.
This is an algebraic observation, not a resolution of the liar paradox;
see Kripke~\cite{kripke1975} and Priest~\cite{priest1979} for semantic
approaches.
...
\item Impossibility results: no negation-preserving collapse to classical
or G\"odel logic, no signature-preserving collapse to \L ukasiewicz logic.
The positive collapse to Kleene is a routine quotient by
$\{0\}, (0,1), \{1\}$; the impossibility results are the substantive content.
...
collapse homomorphism, impossibility results, Bayes consistency, path
dependence, and the RM3/G\"odel comparisons reported in this paper.

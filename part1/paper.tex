...
\item \textbf{Uniqueness of min} (Theorem~\ref{thm:uniqueness-main}):
  The min copula is the unique symmetric, idempotent copula on $[0,1]$
  (a known result~\cite{nelsen2006}; we give a self-contained Lean-verified proof).
...
Section~\ref{sec:conditioning} develops chain-rule conditioning and path dependence (Main Result~2).
...
$[0,1]$~\cite{nelsen2006}.
This section gives a self-contained proof (machine-checked in Lean) and
interprets the result: the minimum conjunction is the only truth-functional
joint compatible with Bayesian conditioning that satisfies idempotence.
...
Proposition~\ref{thm:path-dependence} has the same structure as the
...
When $\cred(B) > 0$\label{thm:existence}\label{thm:uniqueness},
the conditional is uniquely determined by division:
$\cred(A \mid B) = \cred(A \land B) / \cred(B)$, which lies in $[0,1]$
provided $\cred(A \land B) \leq \cred(B)$.
This condition is both necessary and sufficient (since $\text{condCred} \leq 1$).
...
\item Bayes consistency as a selection criterion for arrows: the product
residuated implication passes; RM3 and G\"odel fail.
\end{enumerate}

...
The algebra keeps (1) as its value-level negation law and adopts (2) as the sole
...
logic but essential for understanding why The algebra's chain-rule approach differs
from residuation.
...
The credence algebra sidesteps this: as a pure algebra with no consequence relation, it
replaces ``forced to~$1$'' with ``unconstrained by the chain rule.''

The credence algebra makes this divergence the defining design choice.
...
these axioms, but the algebra requires neither a sigma-algebra nor additivity.
...
The credence algebra thus inherits the product De Morgan triplet from fuzzy logic and the
...
\item A surjective homomorphism from the credence algebra to the Kleene lattice (underlying
...
We call this a \emph{constraint algebra} on $[0,1]$: it specifies how
...
\item It has no \emph{update rule}: when new evidence arrives, the algebra does not prescribe how to revise credences.
\end{itemize}
...
sigma-additivity), the credence algebra provides the algebraic layer that multiple
higher-level systems can build on.

\textbf{Two levels of use.}
The algebra operates at two granularities.
...
Staying algebraic is what allows the collapse homomorphism to connect the credence algebra
...
\emph{The credence algebra is designed to be compatible with both traditions.}
By staying at the algebraic layer---values, operations, and the chain
rule constraint---the algebra takes no position on whether values represent
...
\end{itemize}
The algebra does not commit to any particular dependence structure.
...
The algebra's core consists of three ingredients: values in $[0,1]$, the complement
$\negc c = 1 - c$, and the chain rule for conditioning.
Probability adds a sigma-algebra, a measure, and sigma-additivity; every
probability space therefore satisfies The algebra's axioms, making probability a
special case.
...
The algebra makes no such determination: $c_1 \conjc c_2$ is multiplication on
$[0,1]$, not a claim about the actual joint of arbitrary propositions.
The actual joint is an external input to conditioning.
The algebra's disjunction $c_1 \disjc c_2 = c_1 + c_2 - c_1 c_2$ is the De Morgan
...
In The algebra's graded setting, ``void'' becomes ``any value in $[0,1]$ satisfies
the chain rule.''
...
single extra output.
Void can be viewed as the uniform extra that The algebra's chain rule
...
The algebra leaves the void unresolved---the choice of how to fill it in is
...
The algebra's conditioning is not new machinery---it is probability's chain rule without
...
true and false.
The algebra's $\tfrac{1}{2}$ is simply the unique fixed point of complement on $[0,1]$.
\end{remark}
...
denote The algebra's. Both negations are $1 - c$, but the notational distinction
...
Chain-rule conditioning (with Kleene joint) matches the product residuated for
...
Product fuzzy logic commits to this choice; the credence algebra leaves the joint free.
...
The algebra's chain-rule conditioning replaces with ``unconstrained'') is
...
undefined; The algebra's chain rule $c \cdot 0 = 0$ is satisfied by any $c$,
...
\label{rem:freedom}
The algebra leaves the joint free: any value in the Fr\'echet-Hoeffding interval
...
The independence copula $a \cdot b$ (The algebra's $\conjc$) yields the trivial
arrow $b$.
...
The algebra's parametric conditioning encompasses all of them.
\end{remark}
...
arrow---is an instantiation of The algebra's parametric conditioning.
In this sense the credence algebra is more general than any single t-norm logic in the
...
If the algebra commits to a single binary arrow via residuation (becoming a t-norm
...
Product logic and the credence algebra are thus two views of the same algebraic
...
is unconstrained.
To recover classical logic from the Boolean restriction, one must \emph{add} the axiom
$\cred(A \mid 0) = 1$.

\subsection{Subalgebra vs.\ Quotient}
\label{sec:scope}

The credence algebra relates to classical logic and to the Kleene lattice in structurally
different ways: embedding vs.\ quotient.

\textbf{Boolean subalgebra.}
The set $\{0, 1\} \subset [0,1]$ is closed under all the algebra's operations:
...
has no image in $\{0,1\}$.
The Boolean algebra embeds into the credence algebra as a subalgebra, but there is no
surjection back.

\textbf{Kleene quotient.}
The set $\{0, \tfrac{1}{2}, 1\}$ is \emph{not} closed under the algebra's operations:
$\tfrac{1}{2} \conjc \tfrac{1}{2} = \tfrac{1}{4} \notin \{0, \tfrac{1}{2}, 1\}$.
Intermediate values interact to produce other intermediate values, so the
three-valued algebra cannot live inside the credence algebra as a subalgebra.
...
\label{rem:fde}
The algebra's four-status collapse (true/false/both/void) superficially resembles
...
Belnap-Dunn is intrinsically four-valued; the credence algebra is fundamentally $[0,1]$ with
...
The algebra's ``void'' arises when conditioning on impossible evidence
...
In the credence algebra, $\tfrac{1}{2}$ is the arithmetic midpoint with specific
...
\emph{Conditioning.}
The credence algebra has primitive chain-rule conditioning; Belnap-Dunn has no native
conditioning operation.

\emph{Probabilistic interpretation.}
The credence algebra has the comonotonicity/quantile semantics connecting to probability
...
negation has two fixed points (B and N) while The algebra's complement has one
($\tfrac{1}{2}$), matching Kleene.
The credence algebra and Belnap-Dunn address different concerns: it provides
...
The algebra's axioms, making probability a special case.
...
This is why the credence algebra can serve as an algebraic foundation for multiple systems:
...
The credence algebra provides the unfused algebraic core.
...
What the algebra drops: no measure, no sigma-algebra, no additivity, no event
...
At evidence zero, the algebra leaves the conditional unconstrained rather than
...
Probability is a special case of the credence algebra (same chain rule, additional structure).
The algebra's algebra $\{\negc, \conjc, \disjc\}$ collapses to the Kleene lattice
shared by K$_3$, LP, and RM3.
The algebra's conditioning, when instantiated as a binary arrow via the Kleene joint,
...
the credence algebra leaves the joint free and the evidence-zero row unconstrained.
...
The algebra's unconstrained conditioning applied in a discrete setting
...
\textbf{The algebra's Contributions.}

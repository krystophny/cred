...
product residuated implication as the unique Bayes-consistent binary arrow.
...
The structure $([0,1], \conjc, \disjc, \negc, 0, 1)$ forms a \emph{bounded De Morgan
bisemigroup}: two dual commutative monoids ($([0,1], \conjc, 1)$ and $([0,1], \disjc, 0)$)
connected by an order-reversing involution $\negc$.
...
$f(0, b) = 1$) is Bayes-consistent: $f(a, b) \cdot a = \min(a, b) = f(b, a) \cdot b$.
\end{proposition}
\begin{proof}
...
A surjective homomorphism from Cred to the full four-valued algebra would require
reaching $\mathbf{n}$ (``neither true nor false''), which has no natural preimage
in $[0,1]$.
\end{remark}
...
The formalization comprises approximately 1070 lines covering:
...
\item \textbf{Operations:} \texttt{neg}, \texttt{conj}, \texttt{disj}, \texttt{spread},
\texttt{certainty}, \texttt{collapse}.
...
Applying the chain rule from both directions (Bayes' theorem) selects the
product residuated implication as the unique standard Bayes-consistent binary
arrow on $[0,1]$: RM3's arrow fails even on three values, and G\"odel's

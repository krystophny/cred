...
\begin{abstract}
Can probability and many-valued logic be unified?
We prove they can, and the answer is \textbf{unique}.

\textbf{Main Result 1 (Uniqueness).}
Among all truth-functional conjunctions compatible with probabilistic
marginalization, the minimum $\min(a,b)$ is the \emph{only} one satisfying
Bayes consistency, non-trivial conditioning, and idempotence.
This is not a design choice---it is mathematically forced.
The minimum (Fr\'echet upper bound) explains why K$_3$, LP, and RM3 all use
the Kleene conjunction: it is the unique meeting point of logic and probability.

\textbf{Main Result 2 (Path Dependence).}
As evidence $b \to 0$, the conditional's limit depends on the \emph{path}
by which credences approach zero---the ratio $a/b$, not $b$ alone.
This path dependence has the same structure as the Borel--Kolmogorov paradox.
Consequence: ex falso ($0 \to c = 1$) is \emph{not} entailed by the algebra;
forcing it requires an additional totalization convention.

These results arise from a simple framework: credences in $[0,1]$, the
complement $1-c$, and the chain rule
$\cred(A \mid B) \cdot \cred(B) = \cred(A \land B)$ as the sole structural
axiom for conditioning.
When evidence is positive, division emerges as the unique solution; when
evidence has credence zero, the constraint is satisfied by any value---the
conditional is \emph{unconstrained}.
We establish a surjective homomorphism to the Kleene lattice and prove no
analogous homomorphism exists for \L ukasiewicz or G\"odel algebras.
...
\item Lean~4 formalization of the core algebra (Appendix~\ref{app:lean}).
...
A central finding (Theorem~\ref{thm:uniqueness-main}) is that only one
...
Section~\ref{sec:uniqueness} proves the uniqueness theorem (Main Result~1).
Section~\ref{sec:conditioning} develops primitive conditioning and proves
path dependence (Main Result~2).
Section~\ref{sec:fixed-points} treats fixed points and self-reference.
Section~\ref{sec:connections} relates the algebra to existing frameworks.
Section~\ref{sec:conclusion} concludes.
Appendix~\ref{app:prelim} provides formal preliminaries (t-norms, copulas).
Appendix~\ref{app:logic-tables} contains detailed logic comparison tables.
Appendix~\ref{app:lean} describes the Lean formalization.

%=============================================================================
% PRELIMINARIES MOVED TO APPENDIX A
% See \ref{app:prelim} for formal definitions of t-norms, copulas, and
...
We now prove the second main result: the mathematics \emph{actively argues against}
...
\item[Option B: Totalize to~$1$.]
  ...
  is a \emph{convention}, not entailed by the algebra.
...
\end{description}
We take Option~A: the conditional at zero is void (unconstrained).
...
Note: This table treats the joint as a \emph{free parameter}, not computed from a
copula; for the Kleene joint $\min(a,b)$, see the tables below.}
...
Cred and Belnap-Dunn address different concerns: Cred provides
probabilistically-grounded reasoning with conditioning, while Belnap-Dunn
handles incomplete and contradictory information without probabilistic
semantics.
\end{remark}
...
Section~\ref{sec:uniqueness} established that the minimum conjunction is the
unique truth-functional joint satisfying all reasoning requirements.
Here we develop the probabilistic interpretation.
...
The contrapositive also works: $\cred(H \mid \lnot E) = 0$---if the evidence
fails, the hypothesis is certainly false.
\end{example}
...
This paper establishes two main results about the relationship between
probability and many-valued logic.

\textbf{Main Result 1 (Uniqueness, Theorem~\ref{thm:uniqueness-main}).}
The minimum conjunction $\min(a,b)$ is the \emph{unique} truth-functional
joint compatible with probabilistic marginalization that satisfies Bayes
consistency, non-trivial conditioning, and idempotence.
This explains why K$_3$, LP, and RM3 all use the Kleene conjunction: it is
not a choice among alternatives but the unique meeting point of logic and
probability.

\textbf{Main Result 2 (Path Dependence, Theorem~\ref{thm:path-dependence}).}
The conditional's limit as evidence approaches zero is \emph{path-dependent},
varying with the ratio $a/b$ (equivalently, the approach angle in polar
coordinates).
This is the same structure as the Borel--Kolmogorov paradox.
Ex falso ($0 \to c = 1$) is therefore \emph{not} forced by the algebra;
obtaining it requires adding a totalization convention beyond what the
chain rule entails.

The framework is simple: credences in $[0,1]$, the complement $1-c$, and the
chain rule $\cred(A \mid B) \cdot \cred(B) = \cred(A \land B)$ as the sole
structural axiom for conditioning.
The collapse homomorphism (Theorem~\ref{thm:collapse}) connects this algebra
to the Kleene lattice, with impossibility results for alternative targets.
The main results are machine-checked in Lean~4 (Appendix~\ref{app:lean}).
...
%=============================================================================
\section{Logic Comparison Tables}
\label{app:logic-tables}
%=============================================================================

This appendix contains detailed comparison tables for the various logics
and implications discussed in the paper.

\textbf{Three-valued operations (Kleene lattice):}
...
\textbf{Implication comparison} (rows = evidence $a$, columns = conclusion $b$):

\begin{center}
\begin{tabular}{c|ccc}
\multicolumn{4}{c}{\textbf{Kleene material}: $\max(\negc a, b)$} \\
& 0 & $\tfrac{1}{2}$ & 1 \\
\hline
0 & 1 & 1 & 1 \\
$\tfrac{1}{2}$ & $\tfrac{1}{2}$ & $\tfrac{1}{2}$ & 1 \\
...
\quad
\begin{tabular}{c|ccc}
\multicolumn{4}{c}{\textbf{Chain-rule cond.}: joint $= \min(a,b)$} \\
& 0 & $\tfrac{1}{2}$ & 1 \\
\hline
0 & $*$ & $*$ & $*$ \\
$\tfrac{1}{2}$ & 0 & 1 & 1 \\
...
\textbf{Comparison of joint formulas:}

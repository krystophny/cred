...
We establish a surjective homomorphism from Cred to the three-valued algebra
underlying RM3, prove that negation has a unique fixed point at $\tfrac{1}{2}$,
and provide machine-checked proofs in Lean~4.
\end{abstract}
...
\item A surjective homomorphism from Cred to the three-valued RM3 algebra for
$\{\negc, \conjc, \disjc\}$, with machine-checked proof (Section~\ref{sec:connections}).
...
\subsection{Three-Valued Collapse and RM3}
...
\end{align*}
where the three-valued operations match RM3.
\end{theorem}
\begin{proof}
Case analysis on whether each $c_i$ equals $0$, $1$, or lies in $(0,1)$.
The key cases are:
\begin{itemize}
\item If $c_1 = 0$ or $c_2 = 0$, then $c_1 \conjc c_2 = 0$, matching RM3.
\item If $c_1 = 1$, then $c_1 \conjc c_2 = c_2$, matching RM3.
\item If $0 < c_1, c_2 < 1$, then $0 < c_1 \conjc c_2 < 1$, so both sides collapse to $\tfrac{1}{2}$.
\end{itemize}
...
De Morgan lattices.
RM3~\cite{meyer1972} is a three-valued relevant logic; our collapse homomorphism
connects Cred to its underlying algebra.

\textbf{Paraconsistent Logic.}
Da Costa~\cite{dacosta1974} developed paraconsistent logics tolerating contradiction
without explosion.
Priest~\cite{priest1979} introduced the Logic of Paradox (LP), assigning ``both
true and false'' to paradoxical sentences.
...
\item The explicit three-valued collapse homomorphism to RM3 with Lean
verification.
...
The three-valued collapse establishes a surjective homomorphism to RM3 for
$\{\negc, \conjc, \disjc\}$, while conditioning remains distinct from RM3
implication.

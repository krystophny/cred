...
| Graded atlas seed (1-dim): smoothness status, composition closure, crisp recovery to `ContDiff` | `transitionSmoothness`, `transitionSmoothness_eq_one_iff`, `transitionSmoothness_comp`, `atlasStatus`, `smooth_atlas_recovery` | `Topology/Manifold.lean` | theorem |
| n-D manifold recovery: atlas smoothness status = 1 iff M is a mathlib C-infinity manifold (model `EuclideanSpace R (Fin n)`, order `∞`) | `transitionStatus`, `atlasSmoothStatus`, `allTransitionsSmooth_iff_hasGroupoid`, `atlasSmoothStatus_eq_one_iff_isManifold`, `atlasSmoothStatus_model_space` | `Topology/ManifoldN.lean` | theorem |
| Differential: smoothness status = 1 iff `ContMDiff ∞`; at status 1 `mfderiv` is the genuine tangent pushforward `T_xM →L T_{fx}M'`; bridge to `ContDiff` on the model | `mdiffStatus`, `mdiffStatus_eq_one_iff`, `mdiffStatus_mdifferentiable`, `mdiffStatus_mfderiv_isCLM`, `mdiffStatus_modelMap_eq_one_iff`, `mdiffStatus_comp`, `mdiffStatus_id_mfderiv`, `mdiffStatus_const_mfderiv` | `Topology/Differential.lean` | theorem |
| Tangent bundle: TM is a mathlib smooth manifold; bundle projection smooth; zero section a smooth section (each status = 1, with iff-recovery) | `tangentBundleStatus`, `tangentBundleStatus_eq_one_iff`, `tangentBundleStatus_eq_one`, `tangentProjStatus_eq_one`, `zeroSectionStatus_eq_one`, `zeroSmoothSection`, `*_model_space` | `Topology/TangentBundleLayer.lean` | theorem |
| Lie group: status = 1 iff mathlib `LieGroup`/`LieAddGroup`; smooth add/neg/left+right translation; Euclidean additive group witness | `lieGroupStatus_eq_one_iff`, `lieAddGroupStatus_eq_one_iff`, `lieAddGroupStatus_model_space`, `groupAddStatus_eq_one`, `groupNegStatus_eq_one`, `leftAddTransStatus_eq_one`, `rightAddTransStatus_eq_one` | `Topology/LieGroupLayer.lean` | theorem |
| Integral curves / flows: status = 1 iff `IsIntegralCurve`; Picard-Lindelof existence; uniqueness on a Hausdorff manifold; explicit constant-field translation flow | `integralCurveStatus`, `integralCurveStatus_eq_one_iff`, `integralCurve_exists`, `integralCurve_unique`, `integralCurve_unique_boundaryless`, `constField_integralCurve`, `constField_integralCurve_status` | `Topology/IntegralCurveLayer.lean` | theorem |
| Hamiltonian flow capstone: harmonic-oscillator phase flow `(q0 cos t + p0 sin t, -q0 sin t + p0 cos t)` is an integral curve of `X_H=(p,-q)`, conserves `H=(q^2+p^2)/2`, time-t map symplectic | `hamiltonian`, `hamField`, `flow`, `oscillator_isIntegralCurve`, `oscillator_flow_status`, `oscillator_energy_conserved`, `oscillator_flow_symplectic`, `oscillator_flow_init` | `Topology/HamiltonianFlow.lean` | theorem |
| Diffeomorphisms: status = 1 iff a mathlib `Diffeomorph` witness; forward+inverse `ContMDiff`; identity, CLE, and composition witnesses | `diffeoStatus`, `diffeoStatus_eq_one_iff`, `diffeoStatus_contMDiff`, `diffeoStatus_contMDiff_symm`, `diffeoStatus_id`, `clmDiffeoStatus_eq_one`, `diffeoStatus_comp` | `Topology/DiffeoLayer.lean` | theorem |

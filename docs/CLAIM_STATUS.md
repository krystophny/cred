...
| Tangent bundle layer: TM is a mathlib smooth manifold; projection is smooth; zero section is a smooth section | Lean theorem | `tangentBundleStatus_eq_one`, `tangentProjStatus_eq_one`, `zeroSectionStatus_eq_one` (each with `_eq_one_iff`) | M a C-infinity manifold over EuclideanSpace R (Fin n) |
| Lie group layer: status = 1 iff mathlib LieGroup/LieAddGroup; smooth add/neg/translation; Euclidean group instance | Lean theorem | `lieGroupStatus_eq_one_iff`, `lieAddGroupStatus_eq_one_iff`, `lieAddGroupStatus_model_space`, `groupAddStatus_eq_one`, `groupNegStatus_eq_one`, `leftAddTransStatus_eq_one` | abstract G with instances, or EuclideanSpace R (Fin n) additive |
| Integral-curve/flow layer: status = 1 iff IsIntegralCurve; Picard-Lindelof existence; uniqueness; explicit constant-field translation flow | Lean theorem | `integralCurveStatus_eq_one_iff`, `integralCurve_exists`, `integralCurve_unique`, `constField_integralCurve_status` | existence: CompleteSpace + Boundaryless + C-infinity field; uniqueness: T2 + interior + ContMDiff |
| Hamiltonian flow capstone: harmonic-oscillator phase flow is an integral curve of X_H, conserves energy, time-t map symplectic | Lean theorem | `oscillator_isIntegralCurve`, `oscillator_energy_conserved`, `oscillator_flow_symplectic` | one worked system on R^2; not general symplectic-manifold theory |
| Diffeomorphism layer: status = 1 iff a mathlib Diffeomorph witness; identity/composition; forward and inverse smooth | Lean theorem | `diffeoStatus_eq_one_iff`, `diffeoStatus_id`, `diffeoStatus_comp`, `diffeoStatus_contMDiff`, `diffeoStatus_contMDiff_symm`, `clmDiffeoStatus_eq_one` | maps between Euclidean-model charted spaces, order ∞ |
| Differential forms / de Rham / connections / curvature / Hausdorff measure as graded layers | conjecture / roadmap | (none yet) | future; no mathlib target to recover; see FOUNDATION_TRACK_ROADMAP.md |

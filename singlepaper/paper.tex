...
\paragraph{An atlas seed.}
A first cut toward differential geometry, honest about its scope.  A
graded chart is a coordinate map into the one-dimensional model
$\mathbb{R}$; a supplied transition $\tau:\mathbb{R}\to\mathbb{R}$ carries
a smoothness status, $1$ iff $\tau$ is $C^\infty$
(\Lean{transitionSmoothness\_eq\_one\_iff}), closed under composition
(\Lean{transitionSmoothness\_comp}).  An atlas aggregates per-transition
statuses by the product meet, and the crisp recovery returns every
transition as a genuine smooth map once the aggregate status is $1$
(\Lean{smooth\_atlas\_recovery} in \texttt{Cred.Manifold}).  There is no
cover, Hausdorff, or second-countability condition: those are exactly
what a manifold theory adds, and they are absent on purpose.

These layers share one shape: a degree map $P:X\to[0,1]$ whose value $1$
recovers the classical predicate exactly, machine-checked.  Metric
closeness, the limit, and continuity recover their mathlib counterparts
at the boundary; differentiability and smoothness are two-valued status;
dimension is the named Moran source; the atlas is a labelled seed.  What
remains is genuine manifold theory (charts with cover and Hausdorff
conditions, tangent bundles, differential forms) and measure-theoretic
dimension (Hausdorff measure, Moran's theorem).  The staged plan is in

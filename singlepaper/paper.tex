...
\hypersetup{
  ...
  pdftitle={Credence, Conditioning, and the Logic of Impossible Evidence}
}
...
\title{Credence, Conditioning, and the Logic of Impossible Evidence}
...
formal results are machine-checked in Lean 4 with Mathlib; theorem
...
It gives a precise, non-explosive account of zero evidence:
$\Cond(0,0)=[0,1]$.  This is the mathematical hinge. \\
...
\]
R\'enyi and Popper instead treat conditional probability as primitive
...
indicative conditionals still turns on this boundary between
truth, probability, and undefinedness \cite{egre2025}.
...
\cite{walley1991,troffaes2014}.  A set of possible posteriors appears
only when the supplied constraint fails to determine a point.  The set
is not psychological indecision.  It is the fiber of an equation.
...
\cite{hajek1998,esteva2001,cintula2011}.  The present system keeps the
product and De Morgan complement, but refuses to make residuation the
conditional.  That refusal is not cosmetic: the Curry block shows that
the full residuated package is incompatible with contraction on this
carrier.
...
Primitive conditional probabilities go back to R\'enyi and Popper
\cite{renyi1955,popper1959,krauss1968,dubins1975}.  Imprecise
probability studies sets of probabilities as first-class objects
\cite{walley1991,troffaes2014}.  Disintegration accounts condition by
measures on level sets of a statistic \cite{chang1997}.  Here the
fiber is narrower: $\Cond(j,e)$ is the preimage of one scalar equation.
It becomes wide exactly when evidence is zero.

\paragraph{Conditional-event algebras.}
Gilio and Sanfilippo analyze conditional random quantities and
compounds of conditionals in a coherence setting \cite{gilio2014}.
Flaminio, Godo, and Hosni build Boolean algebras of conditionals and
probability measures over them \cite{flaminio2020conditionals}.  The
present construction is more austere: it keeps only the scalar value,
the supplied joint, and the admissible fiber.

\section{What Is Actually Novel}

The novelty is not the product t-norm, the Kleene lattice, primitive
conditional probability, or paraconsistency.  Those are established.
The publishable contribution is the verified conjunction of five
claims:
\[
\begin{array}{ll}
\text{(N1)} &
\Adm_V(c;j,e)\Longleftrightarrow c\conjc e=j
\text{ as external conditional semantics};\\[1mm]
\text{(N2)} & \Cond(0,0)=[0,1]\text{ as the exact no-ex-falso mechanism};\\[1mm]

...
\section{Graded Structure Status}
...
A numerical scheme that preserves a structure exactly has degree $1$: a
symplectic step keeps the form
(\Lean{symplectic\_exact\_degree\_one}), and likewise a first integral
and finite-volume mass (\texttt{Cred.Approx}; see the inventory).
Near-preservation is reported by the residual-to-score recipe
$\mathrm{score}_\varepsilon(r)=\max(0,1-r/\varepsilon)$
(\Lean{scoreEps}: $\mathrm{score}_\varepsilon(0)=1$, $0$ at $r\ge\varepsilon$,
antitone).  For the explicit Euler step the symplectic residual is
$|\det-1|=h^2$, so the degree is below $1$ for any nonzero step and
collapses to $0$ at the full step
(\Lean{explicitEuler\_degree\_lt\_one}).  The degree is the conservation
residual, not a label.
...
The Cantor set is a graded-structure benchmark with an exact value and a
finite estimate.  Its similarity dimension is the unique Moran solution
of $2\,(1/3)^s=1$, namely $s=\log 2/\log 3$
(\Lean{cantor\_moran}, \Lean{cantor\_moran\_unique} in
\texttt{Cred.Examples.Fractal}); at stage $k$ the box count is $2^k$ and
the finite box-dimension estimate $\log(2^k)/\log(3^k)$ equals
$\log2/\log3$ exactly (\Lean{box\_estimate\_eq\_dim}).  Exact
self-similarity is degree $1$; coarser resolutions report a graded
status.

This is a status layer over classical structures, recovering the
classical predicate at degree $1$.  It is not a development of topology,
differential geometry, or fractal geometry; the staged plan is in

...
inferences are its valuation-level and value-level instances.
\begin{center}
\begin{tabular}{@{}llll@{}}
\toprule
...
\begin{tikzpicture}[>=Latex]
  ...
  \node[draw,rounded corners,fill=blue!8,align=center,
    ...
    {consequence\\points: valuations $v$\\$f=\val{\cdot}_v$,\quad $S={\uparrow}t$};
  \node[draw,rounded corners,fill=red!8,align=center,
        below right=12mm and 2mm of op] (con)
    {conditioning\\points: values $c$\\$f=(\,\cdot\,)\conjc e$,\quad $S=\{j\}$};
  \draw[->] (op) -- (ent);
  \draw[->] (op) -- (con);
...
over valuations, conditioning over values; the chain-rule cut pushes a

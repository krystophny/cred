...
write the fiber
\[
  \Cond_V(j,e)=\{c\in V:\Adm_V(c;j,e)\}.
\]
The paper uses this set notation as metalanguage.
...
\cite{walley1991,troffaes2014}.  Here the imprecision is narrower:
the set $\Cond(j,e)$ is the fiber of one scalar equation.  It becomes
wide exactly when evidence is zero.
...
\text{(N5)} & \text{Curry block explaining why no internal arrow is added.}
\end{array}
\]

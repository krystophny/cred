-- Normalization for CredTT via Tait-style logical relations
--
-- ============================================================================
-- AXIOM STATUS SUMMARY
-- ============================================================================
--
-- This module contains a SKETCH of the normalization proof for CredTT.
...
--
-- 4. CREDENCE-IRRELEVANT-REDUCTION
--    Status: SHOULD BE PROVEN
...
--
-- KEY INSIGHT: Credences do not affect reduction
-- The beta rules are purely syntactic. Credences only determine WHICH terms
-- are well-typed, not how they reduce. Therefore, normalization in CredTT
-- is identical to normalization in MLTT.
--
-- ============================================================================

module CredTT.Normalization where
...
module Normalization {ℓ : Level} (DM : DeMorganAlgebra ℓ) where
  ...
  -- Credences don't affect the syntactic structure, only the typing derivations.
  -- Therefore, normal forms in CredTT are identical to MLTT.
  mutual
  ...
  -- =========================================================================
  -- POSTULATE: Subject Reduction (credence-irrelevant-reduction)
  -- Status: SHOULD BE PROVEN (depends on substitution lemma)
  -- =========================================================================
  --
  -- This states that if t reduces to t' and t is well-typed, then t' is
  -- well-typed (possibly at a different credence). This is the subject
  -- reduction theorem.
  --
  -- Why possibly different credence: Consider (lam A b) applied to a.
  -- If lam A b : A -> B @ c and a : A @ d, then (lam A b) a : B @ c*d.
  -- After beta reduction, b[a] : B[a] @ c*d (same credence).
  -- But the reduction itself does not change the credence.
  --
  ...
  -- =========================================================================
  postulate
    credence-irrelevant-reduction : ∀ {n} {t t' : Tm n} {Γ : Ctx n} {A : Ty n} {c : C} ->
      (t ⟶ t') -> Γ ⊢ t ∶ A 〔 c 〕 -> ∃ λ c' -> Γ ⊢ t' ∶ A 〔 c' 〕
  ...
  -- Corollary: credences don't affect normalization
  -- A term normalizes in CredTT iff it normalizes in MLTT.
  -- This is because reduction is purely syntactic.
  credence-independent-normalization : ∀ {n} {Γ : Ctx n} {t : Tm n} {A : Ty n} {c c' : C} ->
                                       Γ ⊢ t ∶ A 〔 c 〕 ->
                                       Γ ⊢ t ∶ A 〔 c' 〕 ->
                                       WeaklyNormalizing t
  credence-independent-normalization d _ = weak-normalization d

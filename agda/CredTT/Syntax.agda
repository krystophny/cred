...
-- Certainty is credence 1, not a special type.
...
mutual
  data Ty (n : ℕ) : Set where
    ...
    _⇒_  : Ty n → Ty (suc n) → Ty n        -- Pi (dependent function)
    _×'_ : Ty n → Ty (suc n) → Ty n        -- Sigma (dependent pair)
  ...
  data Tm (n : ℕ) : Set where
    ...
    -- Function
    lam   : Ty n → Tm (suc n) → Tm n       -- lam(x:A).b

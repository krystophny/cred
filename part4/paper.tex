...
\paragraph{Sequential measurements wash out the width.}
Now take $n$ readings $x_1, \dots, x_n$ instead of one. The coherence theorem
\texttt{sequential\_eq\_batch} (Section~\ref{sec:iteration}, machine-checked)
says that with the joint model fixed, updating one datum at a time equals
updating the whole batch at once: the intermediate posterior cancels, so
$\cred(\theta \mid x_{1:n})$ is the same object whether assembled in $n$ steps
or in one. Sequential and batch conditioning agree, so we may read $n$ readings
as a single effective measurement of mean $\bar x$ with noise $1/\sqrt{n}$.
...
Bernstein--von Mises behavior of Section~\ref{sec:iteration}: the posterior
